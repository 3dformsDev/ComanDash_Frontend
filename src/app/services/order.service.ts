...
export class OrderService {
  ...
  markAsReadyOrder(orderId: number): Observable<Order> {
    return this.http.put<Order>(`${this.apiUrl}/kitchen/markasready/${orderId}`, {});
  }
  ...
  markAsServedOrder(orderId: number): Observable<Order> {
    return this.http.put<Order>(`${this.apiUrl}/waiter/markasserved/${orderId}`, {});
  }
   ...
   * @returns 
   */
  makeOrderPayment(orderPayment: OrderPayment): Observable<any> { // O un tipo más específico si sabes lo que devuelve
 ...
 */
  cancelOrder(payload: { orderId: number; reason?: string; paymentMethodId?: number }): Observable<any> {
