...
import { OrderService } from '@services/order.service'; '@services/order.service'; // Tu servicio de API
...
export class OrdersEffects {
    ...
    loadOrders$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => // switchMap es ideal para peticiones de lectura
        ...
        this.actions$.pipe(
            ...
            mergeMap(([action, ordersFeature]) => {
                ...
                const orderToSend: Order = {
                    ...
                    ...(order.isAdvancePayment && order.paymentDetails && {
                ...
                return this.ordersService.createOrder(orderToSend).pipe(
                    map((newOrder) => OrdersActions.createOrderSuccess({ order: newOrder })),
                    catchError((error) => of(OrdersActions.createOrderFailure({ error: error })))
                );
            })
        )
    );
    ...
    selectOrderForEdit$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ orderId }) =>
                this.ordersService.getOrderById(orderId).pipe( // <-- Asumo que tienes un método así en tu servicio
                    ...
                    catchError(error => of(OrdersActions.selectOrderForEditFailure({ error })))
                )
            )
        )
    );
